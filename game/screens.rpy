...
init python:
    ...
    # TODO JM also set breadth and width (should this be a class?)
    def set_inventory_mode(mode):
        stretch = 0.6 # distance across (perpendicular to edge)
        breadth = 0.4 # distance from edge
        shower_scale = 0.1

        global inv_area
        global inv_show_area

        # set position relative to side
        if mode is INV_BOTTOM:
            inv_area.y = 1.0 - breadth
        elif mode is INV_TOP:
            inv_area.y = 0
        elif mode is INV_LEFT:
            inv_area.x = 0
        elif mode is INV_RIGHT:
            inv_area.x = 1.0 - breadth
        else:
            raise Exception(f"Unknown inventory mode {mode}")

        # center, set size, and set shower size
        if mode is INV_BOTTOM or mode is INV_TOP:
            inv_area.x = breadth * 0.5
            inv_area.width = stretch
            inv_area.height = breadth
            inv_show_area = Area(inv_area)
            inv_show_area.height *= shower_scale
        elif mode is INV_LEFT or mode is INV_RIGHT:
            inv_area.y = breadth * 0.5
            inv_area.height = stretch
            inv_show_area = Area(inv_area)
            inv_show_area.width *= shower_scale

        # correct position of shower if needed
        if mode is INV_RIGHT:
            inv_show_area.x = 1.0 - inv_show_area.width
        elif mode is INV_BOTTOM:
            inv_show_area.y = 1.0 - inv_show_area.height


screen Inventory():
    zorder ZORDER_INVENTORY
    frame:
        area (inv_area.x, inv_area.y, inv_area.width, inv_area.height)
...
screen InventoryShower():
    frame:
        area (inv_show_area.x, inv_show_area.y, inv_show_area.width, inv_show_area.height)
